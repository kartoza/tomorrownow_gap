...
from datetime import timedelta, datetime, time as time_s
...
class TioShortTermDailyCollector(BaseIngestor):
    ...
    def __init__(self, session: CollectorSession, working_dir: str = '/tmp'):
        ...
        self.forecast_attrs = self.dataset.datasetattribute_set.select_related(
            'attribute'
        ).filter(
        ...
        self.attribute_requests = [
            f.source for f in self.forecast_attrs
        ]
    ...
    async def fetch_producer(self, grid, payload):
        ...
        url = f'{self.base_url}/timelines?apikey={self.api_key}'
        grid_id = grid['id']
        lat = grid['lat']
        lon = grid['lon']
        last_ex = None
    ...
    def run(self):
        ...
        try:
            ...
            logger.error('Collector Tio Short Term failed!', exc_info=True)
            raise Exception(e)
        finally:
            pass
